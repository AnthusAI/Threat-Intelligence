    ...
    .map((section) => {
      ...
      return {
        ...
        researchPackets: researchRuns.map((run) => ({
          ...
          summary: run.packet?.summary ?? run.packet?.assignment_focus ?? run.packet?.assignmentFocus ?? run.packet?.blockedReason ?? run.packet?.blocked_reason ?? null,
          acceptedEvidenceCount: countArray(run.packet?.evidenceItemIds ?? run.packet?.accepted_evidence_ids ?? run.packet?.acceptedEvidenceIds),
