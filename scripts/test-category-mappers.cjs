...
assert.match(schemaSource, /MessageThread:\s*a\s*\n\s*\.model/);
assert.match(schemaSource.match(/Message:\s*a[\s\S]*?ModelAttachment:/)?.[0] ?? "", /threadId:\s*a\.id\(\)/);
assert.match(schemaSource.match(/Message:\s*a[\s\S]*?ModelAttachment:/)?.[0] ?? "", /responseStatus:\s*a\.string\(\)/);
assert.match(schemaSource, /listMessagesByThreadAndSequence/);
assert.match(schemaSource, /listMessagesByResponseTargetStatusAndCreatedAt/);
assert.match(schemaSource, /listMessageThreadsByAnchorAndUpdatedAt/);
