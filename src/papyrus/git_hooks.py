...
def _hook_content(message: str) -> str:
    # Print exactly what the caller asked for (no timestamps, no extra prefixes).
    # Use printf to avoid echo portability pitfalls.
    return (
        "#!/bin/sh\n"
        f"{SIGNATURE}"
        "printf '%s\\n' "
        + _sh_single_quote(message)
        + " >&2\n"
        "exit 1\n"
    )
...
def gitblock_main(argv: list[str] | None = None) -> int:
    ...
    for name in ["pre-commit", "commit-msg"]:
        ...
        if hook_path.exists() and not args.force and not _is_ours(hook_path):
            print(
                f"ERROR: refusing to overwrite existing hook: {hook_path}",
                file=sys.stderr,
            )
...
def gitunblock_main(argv: list[str] | None = None) -> int:
    ...
    for name in ["pre-commit", "commit-msg"]:
        ...
        if not args.force and not _is_ours(hook_path):
            print(
                f"ERROR: refusing to remove existing hook: {hook_path}",
                file=sys.stderr,
            )
...
if __name__ == "__main__":
    # Convenience for `python -m papyrus.git_hooks gitblock ...` if needed.
    cmd = os.path.basename(sys.argv[0])
    if cmd.endswith("gitunblock"):
        raise SystemExit(gitunblock_main(sys.argv[1:]))
    raise SystemExit(gitblock_main(sys.argv[1:]))
