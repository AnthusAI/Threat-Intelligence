...
retrievedAt importRunId importedAt metadata updatedAt
"""
...
COMMENT_FIELDS = """
id subjectKind subjectId subjectLineageId subjectVersionNumber subjectVersionKey subjectStateKey commentKind body status
source importRunId authorSub authorUserProfileId authorLabel metadata createdAt
"""

RELATION_FIELDS = """
id relationState predicate subjectKind subjectId subjectLineageId subjectVersionNumber objectKind objectId objectLineageId
...
""",
    "knowledgeComment": f"getKnowledgeComment(id: $id) {{ {COMMENT_FIELDS} }}",
}
...
LIST_COMMENTS_BY_SUBJECT_STATE_QUERY = f"""
query ListCommentsBySubjectState($subjectStateKey: String!, $limit: Int, $nextToken: String) {{
  listKnowledgeCommentsBySubjectStateAndCreatedAt(subjectStateKey: $subjectStateKey, limit: $limit, nextToken: $nextToken) {{
...
class PapyrusSemanticClient:
    ...
    def list_reference_comments(self, reference_lineage_id: str) -> dict[str, Any]:
        comments = self.list_comments("reference", reference_lineage_id)["comments"]
        return {"comments": comments}

    def list_comments(self, subject_kind: str, subject_lineage_id: str) -> dict[str, Any]:
        comments = self._list_connection(
            LIST_COMMENTS_BY_SUBJECT_STATE_QUERY,
            {"subjectStateKey": semantic_state_key(subject_kind, _required(subject_lineage_id, "subject_lineage_id"))},
            "listKnowledgeCommentsBySubjectStateAndCreatedAt",
        )
        return {"comments": comments}
